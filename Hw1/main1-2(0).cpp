...
int main() {
    ...
    cin >> NumberOfName; //cin >> x
    cout << "MaxLengthOfName:" << endl;
    cin >> MaxLengthOfName; //cin >> y
    cin.get(); // cin.get 吃掉換行
    char **name = (char**) malloc(sizeof(char *) * NumberOfName);
    
    //
    for (int j = 0; j < NumberOfName; j++) {
        cout << "Please enter the name: " << endl;
        cin.getline(name[j] = (char *)malloc(sizeof(char) * (MaxLengthOfName+1)), MaxLengthOfName+1);
    }
